...
if __name__ == "__main__":
    ...
    proj_intr_stuff = np.load(os.path.join(baseDir, calName, "projector_intrinsic_calib.npz"))
    proj_mtx = proj_intr_stuff["mtx"]
    proj_dist = proj_intr_stuff["dist"]       

    cam_R, cam_T = get_extrinsic(objp, checkerboard, dW1, cam_mtx, cam_dist, images[0])  
    proj_R, proj_T = get_extrinsic(objp, checkerboard, dW1, proj_mtx, proj_dist, images[0])  
    ...
    np.savez(os.path.join(baseDir, calName, "projector_extrinsic_calib.npz"), R=proj_R, T=proj_T)
