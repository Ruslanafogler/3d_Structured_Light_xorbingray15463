...
def computeIntrinsic(images, checkerboard, size_of_square, dW):
...
def computeExtrinsic(img_path, mtx, dist, dX, dY):
    ...
    color_img = cv2.resize(color_img, (800, 600))  
    ...
    corner_rays = np.matmul(rmat.T, np.squeeze(10000*pixel2ray(image_corners, mtx, dist)).T)
    print(corner_rays)
